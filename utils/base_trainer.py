...
class BaseTrainer(object):
    ...
    def __init__(self, options):
        ...
        self.options.data_test = self.options.data_test.split('+')
        for d in self.options.data_test:
            self.test_data_loader.append(BaseDataset(None, d, is_train=False))
        ...
        for epoch in tqdm(range(self.epoch_count, self.options.num_epochs), total=self.options.num_epochs, initial=self.epoch_count):
            ...
            for step, batch in enumerate(tqdm(train_data_loader, desc='Epoch '+str(epoch),
                ...
                if time.time() < self.endtime:
                    ...
                    out = self.train_step(batch, epoch)
                    self.step_count += 1
    ...
    def test(self, epoch, step):
        ...
        self.model.eval()
        if self.options.model == "ir_depth_pm_fusion":
            pretrained_ir_depth_model=self.pretrained_ir_depth_model
            self.pretrained_ir_depth_model.eval()
        else:
            pretrained_ir_depth_model=None

        for i, d in enumerate(self.options.data_test):
            run_evaluation(self.model, d, self.test_data_loader[i], 
                           self.options.name, self.options.checkpoint_dir, epoch, step, 
                           model_name=self.options.model, no_render=self.options.no_render, 
                           num_cas_iters=self.options.num_cas_iters, 
                           mod1_epoch=self.options.mod1_epoch,
