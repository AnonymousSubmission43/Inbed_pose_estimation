        ...
        for seq_i in seq_list:
            ...
            for j, vid_i in enumerate(vid_list):
                ...
                for i, img_i in enumerate(img_list):
                    ...
                    # print(np.min(S17[:, 0]), np.max(S17[:, 0]))
                    # print(np.min(S17[:, 1]), np.max(S17[:, 1]))
                    # print(np.min(S17[:, 2]), np.max(S17[:, 2]))
                    ...
                    print(img_view)
                    img_ori = cv2.imread(os.path.join(MPI_INF_3DHP_ROOT,img_view)).copy()
                    scale_rate = 0.3
                    width = int(img_ori.shape[1] * scale_rate)
                    height = int(img_ori.shape[0] * scale_rate)
                    dim = (width, height)
                    img_ori = cv2.resize(img_ori, dim)
                    part[:,:2] = part[:,:2] * scale_rate
                    print(part.shape, width, height)
                    debugger = Debugger()
                    # debugger.add_img(img_ori)
                    # debugger.add_point_2d(part[:,:2], (255, 0, 0))
                    # debugger.add_point_3d(S[:,:3], 'b')
...
if __name__ == '__main__':
    MPI_INF_3DHP_ROOT = '/media/yuyin/10THD1/Dataset/pose/mpi_inf_3dhp'
    out_path = './'
    out_name = 'mpi_inf_3dhp.npz'
    openpose_path = 'datasets/openpose'
    mpi_inf_3dhp_extract(MPI_INF_3DHP_ROOT, openpose_path, out_path, 'train', extract_img=False, static_fits=None)
